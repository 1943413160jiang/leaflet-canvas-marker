...
function layerFactory(L) {
  var CanvasIconLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    initialize: function (options) {
      ...
      this._onMousemoveListeners = [];
      this._onMouseoverListeners = [];
      this._mouseTouch = { isDown: false };
    },
    ...
    onAdd: function (map) {
      ...
      map.on("mousedown", this._executeListeners, this);
      map.on("mouseup", this._executeListeners, this);
    ...
    onRemove: function (map) {
      ...
      map.off("mousedown", this._executeListeners, this);
      map.off("mouseup", this._executeListeners, this);
    ...
    _drawImage: function (marker, pointPos) {
      ...
      const x = pointPos.x - options.iconAnchor[0];
      const y = pointPos.y - options.iconAnchor[1];
    ...
    _drawText(text, x, y, size) {
      ...
      _context.fillText(text, x - size, y + size * 2);
    },
    ...
    addOnMousemoveListener: function (listener) {
      this._onMousemoveListeners.push(listener);
    },
    ...
    _executeListeners: function (event) {
      ...
      if (ret && ret.length > 0) {
        ...
        if (event.type === "mousemove") {
          me._onMousemoveListeners.forEach(function (listener) {
            listener(event, ret);
          });
        }
